...
def send_one_worker(args, df, size):
...
def send_multi_worker(args, df, size):
    pass
    # dfs = [
...
def main():
    ...
    if args.send:
        if not args.workers:
            send_one_worker(args, df, size)
        else:
            if args.workers < 1:
                raise ValueError('workers must be >= 1')
            send_multi_worker(args, df, size)

        cpu_table.block_until_rowcount(args.row_count)
    else:
